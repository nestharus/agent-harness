...
- The user-facing application is a single-user desktop app, supporting `P15` (local control) and `P16` (single-user desktop scope). Tooling/library choices for the desktop shell, frontend SPA, and backend runtime are NOT design decisions and are NOT enumerated here; they are captured in [`/DECISIONS.md`](../DECISIONS.md) (D2 application substrate from `~/projects/agent-runner`; D3–D17 frontend SPA substrate from `~/projects/server-manager` research; D7–D7e and D8–D11 SPA cascade). This proposal references the substrate by category; concrete versions, crates, and packages are roadmap- and DECISIONS-layer concerns per `proposer.md`.
- Model role assignments (orchestrator, optimizer, sub-agent dispatch) are captured in `/DECISIONS.md` D18 and indexed against `~/ai/models/roles.md`. They are not duplicated here. This grounds model use without inventing a separate agent platform, matching `P8`, `P10`, `P12`, and `P16`.
