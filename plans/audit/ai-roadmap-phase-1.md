...
- **Decision register addendum.** Dispatch the three risk gates next on this branch. Do not dispatch them from the proposer. Gate focus should be: (1) whether affected WUs are truly thin/read-only, (2) whether SessionOverrideContract dependencies are neither missing nor over-broad, and (3) whether block-on annotations are precise enough for future VS-015/VS-018 owners.

### Round 4 (brownfield cascade, 56 WUs unchanged)

**Proposer dispatch summary.** proposal-r6 / engineering-roadmap-r5 plus the Phase 0C-r5 concurrent-dispatch direction make SessionOverrideContract v2-only, drop WU-0C-N4, and remove the prior block-on annotations after the agent-runner feature requests landed. Phase 1 r4 seeds from r3 and keeps the same 56-WU inventory.

- **D1 assessment: LOW.** No WU is added, removed, merged, or collapsed. The same 13 r3 cascade-affected WUs retain their Phase 1 read-only boundaries: WU-1-03/04/05/10/12/13/23/25/28/45/48/49/50.
- **D2 assessment: LOW.** The r3 boundary criteria remain intact. Revision rationale on the 13 affected WUs now records the r4 cascade: agent-runner feature requests have landed, SessionOverrideContract is v2-only, and block-on annotations are removed.
- **D3 assessment: LOW.** WU-0C-N4 dependencies are removed; schema-probe references move to WU-0C-N3 `schema_version_probe`. The Phase 1-local topology and Parallelization Map remain unchanged at 7 waves, 18+12+9+5+7+3+2 = 56.
- **Watch signals.** `fix-created-family` remains generation 0 in the Phase 1-local loop and externally driven. No same-family local oscillation is introduced by r4.
- **Decision register addendum.** This is a small-scope brownfield revision only. No commit or push requested.
