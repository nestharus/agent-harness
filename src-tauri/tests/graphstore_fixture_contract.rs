...
async fn empty_seed_plan_creates_isolated_schema_versions_fixture() {
    ...
    assert_eq!(fixture.pool.migrations_applied, vec![1, 4]);
...
async fn reset_preserves_schema_versions_history_exactly() {
    ...
    assert_eq!(rerun.skipped_versions, vec![1, 4]);
...
async fn fixtures_created_in_same_test_have_independent_temp_paths() {
    ...
    assert_eq!(migration_records(&first.pool.sqlite).await.len(), 2);
    assert_eq!(migration_records(&second.pool.sqlite).await.len(), 2);
}
