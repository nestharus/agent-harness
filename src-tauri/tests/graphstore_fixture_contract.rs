...
async fn reset_preserves_schema_versions_history_exactly() {
    ...
    assert_eq!(rerun.skipped_versions, vec![1, 4, 5, 6, 7]);
...
async fn fixtures_created_in_same_test_have_independent_temp_paths() {
    ...
    assert_eq!(migration_records(&first.pool.sqlite).await.len(), 5);
    assert_eq!(migration_records(&second.pool.sqlite).await.len(), 5);
}
